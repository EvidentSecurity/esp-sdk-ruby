  ...
  class RegionStatsApi
    ...
    def list_with_http_info(stat_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: RegionStatsApi.list ..."
      end
      # verify the required parameter 'stat_id' is set
      fail ArgumentError, "Missing the required parameter 'stat_id' when calling RegionStatsApi.list" if stat_id.nil?
      # resource path
      local_var_path = "/api/v2/stats/{stat_id}/regions.json_api".sub('{format}','json').sub('{' + 'stat_id' + '}', stat_id.to_s)
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: RegionStatsApi#list\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
